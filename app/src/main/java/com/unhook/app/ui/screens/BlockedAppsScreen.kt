    ...
    val installedApps by produceState<List<InstalledAppInfo>>(initialValue = emptyList()) {
        value = withContext(Dispatchers.IO) {
            ...
            pm.getInstalledApplications(PackageManager.GET_META_DATA)
                .filter { appInfo ->
                    // Keep user-installed apps AND pre-installed apps that have been
                    // updated via Play Store (FLAG_UPDATED_SYSTEM_APP) — this includes
                    // YouTube, Instagram, etc. which ship with the ROM but are updatable.
